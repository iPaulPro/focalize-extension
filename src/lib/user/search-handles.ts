...
import {SearchProfilesDocument, SearchRequestTypes} from '../graph/lens-service';
import {GraphQLClient} from 'graphql-request';
import {LENS_API} from '../../config';

let searchAbortController: AbortController;

const searchProfiles = async (query: string, limit: number = 5): Promise<Profile[]> => {
    ...
    const {search} = await client.request({document: SearchProfilesDocument, variables: {request}, signal: searchAbortController.signal});
...
export const searchHandles = (query: string, limit: number, cb: (profiles: Profile[]) => void) =>
    searchProfiles(query, limit).then(profiles => cb(profiles)).catch(() => {});