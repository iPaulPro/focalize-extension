<script lang="ts">
    ...
    import {
        author, collectFee, content, cover, description, fileAttachment, draftId, title, urlAttachment,
    ...
    const isMediaPostType = (): boolean =>
        postType === PublicationMainFocus.Audio ||
    ...
    const buildMediaMetadata = (): PublicationMetadataMediaInput => {
        let mediaMetadata: PublicationMetadataMediaInput;

        if ($fileAttachment) {
            mediaMetadata = {
                item: `ipfs://${$fileAttachment.cid}`,
                type: $fileAttachment.type,
            };
        } else if ($urlAttachment) {
            mediaMetadata = $urlAttachment;
        }
    ...
    const buildMetadata = (): PublicationMetadataV2Input => {
        ...
        if ($fileAttachment?.type.startsWith('image/') || $urlAttachment) {
        ...
        } else if ($fileAttachment?.type.startsWith('video/')) {
        ...
        } else if ($fileAttachment?.type.startsWith('audio/')) {
    ...
    $: shouldDisableSubmitBtn = isSubmittingPost ||
        ...
        (isImagePostType && (!$fileAttachment || $urlAttachment));
    ...
    $: {
        ...
        if ($urlAttachment && $fileAttachment) {
            removeAttachmentDialog?.showModal();
        }

        if ((!$fileAttachment && !$urlAttachment) && isMediaPostType()) {
